...
def get_database():
    ...
    return client['bocachicaroadcloasure']
    
def get_data_old_date(client):
    db_infos = client["RoadCloasure"].find()
...
def insert_new_road_cloasure(client, df):
    ...
    df_data_old = pd.DataFrame(list(client["RoadCloasure"].find({})))

    list_id_new = []
    for row in df.to_dict(orient='records'):
        result = client["RoadCloasure"].replace_one(
    ...
    list_id_change = []
    df_data = pd.DataFrame(list(client["RoadCloasure"].find({})))
...
def get_rc_with_id(client, ids, created):
    rcs = client["RoadCloasure"].find({'_id' : {"$in": ids}})
...
def get_rc_to_check(client):
    rcs = client["RoadCloasure"].find({'Flight': -1})
    result = []
    for rc in rcs:
...
def flight_update(client, df):
    for row in df.to_dict(orient='records'):
        client["RoadCloasure"].update_many(
