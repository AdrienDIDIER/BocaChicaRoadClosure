...
def tweet_road_closure_without_api(driver, df):
    ...
    for _, row in df.iterrows():
        ...
        if row['created'] == True:
            img_tmp = Image.open(os.getcwd() + "/images/Road_new.png")
            img = img_tmp.copy()
            img_tmp.close()
        elif "Canceled" in row["Status"]:
            img_tmp = Image.open(os.getcwd() + "./images/Road_canceled.png")
            img = img_tmp.copy()
            img_tmp.close()
        else:
            img_tmp = Image.open(os.getcwd() + "./images/Road_update.png")
            img = img_tmp.copy()
            img_tmp.close()
        ...
        title_font = ImageFont.truetype(os.getcwd() + './fonts/dejavu-sans.book.ttf', 60)
